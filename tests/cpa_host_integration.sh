...
env -u GOROOT PATH="$(dirname "${GO_BIN}"):${PATH}" \
  ...
  "${GO_BIN}" build -trimpath -buildmode=c-shared \
    -ldflags "-s -w -X commandcode-go-cliproxyapi/internal/buildinfo.Version=0.2.6-dev.1 -X commandcode-go-cliproxyapi/internal/buildinfo.Commit=cpa-host-integration" \
